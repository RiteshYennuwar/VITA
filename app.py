...
from config import PINECONE_API_KEY, PINECONE_ENVIRONMENT, COHERE_API_KEY, PINECONE_INDEX_NAME, GEMINI_API_KEY
import markdown
...
def upload():
    if request.method == 'POST':
        try:
            ...
            # Summarize text chunks using GEMINI API
            summarized_texts = [summarize_chunk(chunk) for chunk in text_chunks]
            summarized_text = ' '.join(summarized_texts)
            html_output = markdown.markdown(summarized_text)
            flash('Text summarized successfully!', 'info')
...
def summarize_chunk(chunk):
    """
    Summarize a text chunk using GEMINI API.
